...
local function output_image(card)
    ...
    if output_images and G.ASSET_ATLAS and G.ASSET_ATLAS[card.atlas] and G.ASSET_ATLAS[card.atlas].image_data then
        ...
        local loc_name = localize({ type = 'name_text', set = card.set, key = card.key }):gsub('[<>:"/\\|%?%*]', "_")
        :gsub("[%. ]+$", "")
        local file_path = output_root ..
            "images/" ..
            (loc_name ~= "ERROR" and loc_name or has_loc and G.localization.descriptions[card.set][card.key].name or card.key) ..
            " (" ..
            (card.mod and (card.mod.id == "Balatro" and "Balatro" or SMODS.Mods[card.mod.id].name) or "") .. ").png"
